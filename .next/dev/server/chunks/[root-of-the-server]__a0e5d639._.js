...
"[project]/app/api/emergency-advice/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        console.log("API Key available:", ("TURBOPACK compile-time truthy", 1) ? "Yes (length: " + apiKey.length + ")" : "TURBOPACK unreachable");
        if ("TURBOPACK compile-time falsy", 0) //TURBOPACK unreachable
        ;
        // Get recent reports for context
        const recentReports = __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$mock$2d$data$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["mockReports"].filter((r)=>r.status === "open" || r.status === "in_progress").slice(0, 15).map((r, idx)=>({
                id: idx + 1,
                type: r.type,
                severity: r.severity,
                title: r.title,
...
${recentReports.map((r)=>`Report #${r.id} [${r.type.toUpperCase()}] - ${r.severity.toUpperCase()} SEVERITY
Location: ${r.location}
Coordinates: Available for map display
Issue: ${r.title}
Current Status: Active
Safety Advice: ${r.advice}`).join("\n\n")}
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a0e5d639._.js.map